...
As Depandabot does not support upgrading dependencies from private packages registries yet (It doest now, but it opens one PR for each dependency), I needed to came up with a solution to do it on my own. Lets start right with the code:
